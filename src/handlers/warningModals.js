  ...
  try {
    ...
    if (!parsed) {
      return await interaction.reply({
        embeds: [errorEmbed('❌ Error', 'Invalid interaction data.')],
        flags: ['Ephemeral']
      });
    }

    const { targetUserId, originalModeratorId } = parsed;
  ...
  try {
    ...
    await interaction.editReply({
      embeds: [
        successEmbed(
          '✅ Warnings Cleared',
          `All warnings for **${targetName}** have been cleared. **${result.count}** warning(s) removed.`
        )
      ]
    });
